...
function macos_messageBox()
{
    ...
    {
        // Write the user pasteboard via the in-session kvmagent's native clipboard
        // socket (see getClipboard for why - avoids the ~0.8s-per-call shell spawn
        // that blocked the KVM relay). Protocol: send 'W'[u32 len][len UTF8], recv 'K'.
        {
            var ret = new promise(function (res, rej) { this._res = res; this._rej = rej; });
            try
            {
                var payload = Buffer.from(clipText, 'utf8');
                var hdr = Buffer.alloc(5);
                hdr.writeUInt8(0x57, 0);             // 'W'
                hdr.writeUInt32BE(payload.length, 1);
                var msg = Buffer.concat([hdr, payload]);
                var client = require('net').createConnection({ path: '/tmp/meshagent-clip.sock' }, function ()
                {
                    this.write(msg);
                });
                client.promise = ret;
                client.on('data', function (chunk) { this.promise._res(); try { this.end(); } catch (e) { } });
                client.on('error', function (e) { this.promise._rej(e); });
            }
            catch (e) { ret._rej(e); }
            return (ret);
        }
    ...
    {
        // Read the user pasteboard via the in-session kvmagent's native clipboard
        // socket. The kvmagent runs in the Aqua session where NSPasteboard is a
        // few ms; the root daemon reaching pasteboardd via pbpaste/su- cost ~0.8s
        // per call, which blocked the KVM relay under the viewer's ~1s auto-poll.
        // Protocol: send 'R', receive [u32 changeCount][u32 len][len UTF8 bytes].
        {
            var ret = new promise(function (res, rej) { this._res = res; this._rej = rej; });
            try
            {
                ...
                {
                    this._buf = (this._buf == null) ? chunk : Buffer.concat([this._buf, chunk]);
                    if (this._buf.length < 8) { return; }
                    // [u32 changeCount][u32 len][len UTF8]; changeCount unused here -
                    // the viewer dedupes (message.data != deskLastClipboardReceived)
                    // and only applies when focused, so it is the correct authority.
                    var len = this._buf.readUInt32BE(4);
                    if (this._buf.length < (8 + len)) { return; }
                    var data = this._buf.slice(8, 8 + len).toString('utf8');
                    try { this.end(); } catch (e) { }
                    this.promise._res(data);
                });
                client.on('error', function (e) { this.promise._rej(e); });
            }
            catch (e) { ret._rej(e); }
