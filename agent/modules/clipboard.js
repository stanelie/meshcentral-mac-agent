...
{
    // macOS/FreeBSD always take the direct module.exports.read() path below
    // regardless of console uid, so resolving it first (consoleUid() + getUid()
    // on darwin each spawn a /bin/sh child - two avoidable process spawns per
    // call) is pure wasted work. Skip straight to the direct read.
...
{
    // See dispatchRead(): macOS/FreeBSD never need the resolved uid, so skip
    // the two avoidable /bin/sh spawns (consoleUid() + getUid()) on darwin.
    if (process.platform == 'darwin' || process.platform == 'freebsd')
    {
