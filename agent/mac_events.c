...
static void post_init_once(void *ctx)
{
    ...
    // NOTHING that calls into CoreGraphics/SkyLight may go here.
    //
    // This function body runs inside dispatch_once_f(&g_postOnce). Calling
    // release_stuck_modifiers_locked() from here deadlocked the LOGIN-WINDOW
    // agent outright -- sampled on an affected Mac 2026-09-15:
    //
    //     _dispatch_once_callout
    //       -> SLEventSourceFlagsState (SkyLight)
    //         -> CGSEventSourceForID
    //           -> CGSEventSourceShutdown      [__psynch_mutexwait]
    //
    // The agent reached "kvm_init OK" and then stopped forever: no frames, no
    // input, a black screen in the viewer, while Apple's own Screen Sharing to
    // the same machine worked fine. The trigger is kvm_reset_modifiers() on
    // viewer attach, which primes this once-token -- so it fired on every
    // login-window connection.
    //
    // The release now happens in post_cgevent, AFTER the once has completed.
}
...
static void post_cgevent(CGEventRef e)
{
    ...
    // Once per process, and deliberately out here rather than in post_init_once:
    // see the note there. By this point the once has returned, so SkyLight calls
    // are not being made from inside its callout.
    static int released = 0;
    if (!released) { released = 1; release_stuck_modifiers_locked(); }
...
static void release_stuck_modifiers_locked(void)
{
    ...
    // NOTE: this used to log CGEventSourceFlagsState(kCGEventSourceStateHIDSystemState)
    // first, as evidence of whether a modifier was really latched. That query is
    // what deadlocked the login-window agent (see post_init_once), and it is not
    // worth a black screen: the FLAG-LEAK line in kvm_apply_mod_flags reports the
    // same condition from flags we already own, without asking SkyLight anything.
...
void vnc_probe_once(void)
{
    ...
    // The handshake alone is not enough: it covers CAPTURE only.
    //
    // Measured 2026-09-10, install-time probe vs first pre-login connect:
    //     13:06:04  kTCCServiceScreenCapture|com.apple.screensharing.agent  <- the probe
    //     13:12:55  kTCCServicePostEvent    |com.apple.screensharing.agent  <- still ambushed
    // ScreensharingAgent asks about controlling the screen separately, and only
    // once something actually sends it an input event -- which a handshake-only
    // probe never does, but the agent does the moment it drives the login window.
    //
    // So send one real KeyEvent. Shift alone is the safe choice: it is a
    // genuine input event, but pressing and releasing a bare modifier produces
    // no character and has no side effect, which matters because the operator
    // is sitting in a terminal running the installer while this happens.
    {
        const uint32_t XK_Shift_L = 0xFFE1u;
        uint8_t msg[8];
        for (int down = 1; down >= 0; down--)
        {
            msg[0] = 4; msg[1] = (uint8_t)down; msg[2] = 0; msg[3] = 0;
            msg[4] = (XK_Shift_L >> 24) & 0xFF; msg[5] = (XK_Shift_L >> 16) & 0xFF;
            msg[6] = (XK_Shift_L >>  8) & 0xFF; msg[7] = (XK_Shift_L      ) & 0xFF;
            if (vnc_write_all(g_vnc_fd, msg, 8) < 0) break;
            usleep(150000);
        }
    }
